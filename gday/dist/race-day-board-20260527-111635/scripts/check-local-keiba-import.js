...
const csvPath = path.join(rootDir, "data", "incoming", "local-keiba", ym, `${ym}_events.csv`);
const checks = [];
...
check("Category is local-keiba", allRowsMatch("category", "local-keiba"));
...
if (failed.length > 0) {
  console.error(`\nLocal keiba import check failed: ${failed.length} issue(s)`);
  process.exit(1);
}

console.log(`\nLocal keiba import check passed: ${checks.length} checks`);
