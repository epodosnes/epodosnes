...
const csvPath = path.join(rootDir, "data", "incoming", "boat", ym, `${ym}_events.csv`);
const checks = [];
...
check("Category is boat", allRowsMatch("category", "boat"));
...
if (failed.length > 0) {
  console.error(`\nBoat import check failed: ${failed.length} issue(s)`);
  process.exit(1);
}

console.log(`\nBoat import check passed: ${checks.length} checks`);
