...
const requiredFiles = [
  ...
  "scripts/package-web-release.js",
...
check("Web release package excludes scripts", readText("scripts/package-web-release.js").includes('"scripts"'));
check("Runner has web release command", readText("scripts/run.ps1").includes('"web-release"'));

const latestRelease = getLatestRelease();
const latestWebRelease = getLatestRelease("race-day-board-web");
...
if (latestWebRelease) {
  check(
    "Web release zip exists",
    fs.existsSync(latestWebRelease.zipPath),
    path.basename(latestWebRelease.zipPath),
  );
  const manifest = readJson(latestWebRelease.manifestPath);
  const releaseFiles = Array.isArray(manifest.files) ? manifest.files.map((file) => file.path) : [];
  check("Web release manifest has files", releaseFiles.length > 0, `${releaseFiles.length} files`);
  check("Web release includes index", releaseFiles.includes("index.html"));
  check("Web release excludes scripts", !releaseFiles.some((file) => file.startsWith("scripts/")));
}
...
function getLatestRelease(prefix = "race-day-board") {
  ...
  return {
    ...
    manifestPath: path.join(distRoot, name, prefix.endsWith("-web") ? "web-release-manifest.json" : "release-manifest.json"),
  };
}
