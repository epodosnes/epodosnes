...
const csvPath = path.join(rootDir, "data", "incoming", "auto", ym, `${ym}_events.csv`);
const checks = [];
...
check("Category is auto", allRowsMatch("category", "auto"));
...
if (failed.length > 0) {
  console.error(`\nAuto import check failed: ${failed.length} issue(s)`);
  process.exit(1);
}

console.log(`\nAuto import check passed: ${checks.length} checks`);
