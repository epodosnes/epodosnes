...
const csvPath = path.join(rootDir, "data", "incoming", "keirin", ym, `${ym}_events.csv`);
const checks = [];
...
check("Category is keirin", allRowsMatch("category", "keirin"));
...
if (failed.length > 0) {
  console.error(`\nKeirin import check failed: ${failed.length} issue(s)`);
  process.exit(1);
}

console.log(`\nKeirin import check passed: ${checks.length} checks`);
